    ...
    private void renderSubdividedSegment(
        ...
        if (needsSplit) {
            ...
            if (trailSettings.enableRandomWidth()) {
                scaleStart = scaleStart * (float) trailSettings.randomWidthVariation() * ((perlinNoise.get(startPos.x , startPos.y, startPos.z)) + 1);
                scaleEnd = scaleEnd * (float) trailSettings.randomWidthVariation() * ((perlinNoise.get(endPos.x, endPos.y, endPos.z)) + 1);
